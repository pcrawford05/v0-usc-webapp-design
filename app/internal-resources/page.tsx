...
import { ChevronLeft, Calendar, Users, Star } from "lucide-react"
...
export default function InternalResources() {
  ...
  const handleSearch = ({ query, category }: SearchParams) => {
    ...
    if (category !== "all" && category !== lastSearchParams.category) {
      // Check category before calling toLowerCase
      ...
      )?.resourceType; // This component uses category
    ...
    if (category && category !== "all") {
      ...
      filtered = filtered.filter(
        (group) =>
          // Check category before toLowerCase
    ...
    return (
      <div className="min-h-screen p-6 md:p-12 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/?toggle=basic-search" className="mt-4 inline-flex items-center text-primary hover:text-primary/80 transition-colors">
    ...
    <main className="min-h-screen p-6 md:p-12 bg-white">
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center mb-8">
          <Link href="/?toggle=basic-search" className="flex items-center text-primary hover:text-primary/80 transition-colors">
            <ChevronLeft className="h-4 w-4 mr-2" />
            Back to Home
          </Link>
        </div>
        ...
        <div className="mb-8">
          <h1 className="text-4xl md:text-5xl font-bold mb-4 text-primary">Internal Resources</h1>
          <p className="text-lg text-muted-foreground">
            Explore USC's entrepreneurship resources organized by category
          </p>
        </div>
                            ...
                            <div className="space-y-2">
                              ...
                              {resource.importantDates && (
                                <div className="flex items-start text-sm">
                                  ...
                                  <span>{resource.importantDates}</span>
                                </div>
                              )}
                            </div>
                            <button
